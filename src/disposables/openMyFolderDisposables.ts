...
export const openMyFolderDisposables = vscode.commands.registerCommand(
  "extension-practice.openMyFolder",
  async () => {
    ...
    const selectedMyFolder = myFolders.find(
      (myFolder) => myFolder.name === selectedName,
    );
    if (!selectedMyFolder) {
      return;
    }
    ...
    await vscode.commands.executeCommand(
      "vscode.openFolder",
      vscode.Uri.file(selectedMyFolder.path),
      true,
    );
  },
);
