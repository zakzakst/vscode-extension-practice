...
export function activate(context: vscode.ExtensionContext) {
  console.log(
    'Congratulations, your extension "extension-practice" is now active in the web extension host!',
  );
  const disposable = vscode.commands.registerCommand(
    "extension-practice.helloWorld",
    () => {
      vscode.window.showInformationMessage(
        "Hello World from extension-practice in a web extension host!",
      );
    },
  );

  const countDisposable = vscode.commands.registerCommand(
    "extension-practice.countSelectedText",
    () => {
      ...
      vscode.window.showInformationMessage(
        `選択文字数：${selectedText.length}`,
      );
    },
  );

  const uppercaseDisposable = vscode.commands.registerCommand(
    "extension-practice.toUppercase",
    async () => {
  ...
  const clickedDisposable = vscode.commands.registerCommand(
    "extension-practice.clicked",
    (myFolder: { label: string; path: string }) => {
      vscode.window.showInformationMessage(
        `${myFolder.label}がクリックされました${myFolder.path}`,
      );
    },
  );

  const openGoogleDisposable = vscode.commands.registerCommand(
    "extension-practice.openGoogle",
    async () => {
      await vscode.env.openExternal(vscode.Uri.parse("https://www.google.com"));
    },
  );

  const showCurrentFolder = vscode.commands.registerCommand(
    "extension-practice.currentFolder",
    () => {
  ...
  vscode.window.registerTreeDataProvider("myFoldersView", provider);
}
